			...
			<form id="form-projeto" method="post">
				...
				<div class="modal-body">
					...
					<div class="col-lg-2 group">
						...
						<input type="text" class="form-control" id="dataInicio" name="dataInicio"  value = "<fmt:formatDate value="${dataInicio}" pattern="dd-MM-yyyy" />"/>
			    	<!-- <input type="text" class="form-control" id="dataInicio" name="dataInicio" placeholder="dd/mm/aaaa"> -->
					</div>

					<div class="col-lg-2 group">
						<label for="dataTermino">Data de término</label>
						<input type="text" class="form-control" id="dataTermino" name="dataTermino"  value = "<fmt:formatDate value="${dataTermino}" pattern="dd-MM-yyyy" />"/>
					...
					<div class="col-lg-2 group">
						...
						<input type="text" class="form-control" id="dataRealTermino" name="dataRealTermino"  value = "<fmt:formatDate value="${dataRealTermino}" pattern="dd-MM-yyyy" />"/>
					...
					<div class="col-lg-4 group">
						...
						<select id="status" name="status" class="form-control">
							<c:forEach items="${projeto}" var="status">
					...
					<div class="col-lg-2 group">
						...
						<select id="classificacaoRisco" name="classificacaoRisco" class="form-control">
							<c:forEach items="${projeto}" var="risco">
								<option value="${risco.id}">${risco.classificacaoRisco}</option>
							</c:forEach>											
						</select>
					</div>		
					<div class="col-lg-3 group"></div>		
					...
					<div class="col-lg-2 group">
                        ...
                        <fmt:formatNumber value="${projeto.orcamentoTotal}" type="currency"/>
                    ...
                    <div class="col-lg-2 group">
						...
						<select id="gerenteProjeto" name="gerenteProjeto" class="form-control">
							<c:forEach items="${projeto}" var="gerente">
								<option value="${gerente.id}">${gerente.gerenteProjeto}</option>
							</c:forEach>											
						</select>
                    </div>
                    <div class="col-lg-12 group panel panel-default">
                    	<div class="col-lg-8 group">
                           <label for="equipe">Equipe do projeto</label>
						   <select id="equipe" name="equipe" class="form-control">
								<c:forEach items="${projeto}" var="equipepj">
									<option value="${equipepj.id}">${equipepj.equipe}</option>
								</c:forEach>											
							</select>
							<button type="button" id="btn-incluir" class="btn btn-default" data-dismiss="modal">Incluir</button>
                    	</div>
                    	<div class="col-lg-8 group">
